...
This document defines the naming conventions, code style, commit message format, and document naming conventions used in the project, along with instructions for installing and using `clang-format`. The goal is to make sure code contributed by team members stays consistent in form and easy to track through search tools, code review, and version control. These are the takeaways from finishing the Alien Shooter game — feel free to refer to them and apply whatever fits.
...
|---|---|---|---|
| `lower_snake_case` | Lowercase letters, words separated by underscore `_` | `is_boss_level`, `g_score` | Variables, functions, typedefs, source file names, folder names |
| `UPPER_SNAKE_CASE` | Uppercase letters, words separated by underscore `_` | `MAX_P_BULLETS`, `AC_DISPLAY_GAME_UPDATE`, `AC_TASK_DISPLAY_ID` | `#define` constants, signal enums, task IDs, macros |
...
application/sources/app/
  game/
    game_alien_shooter/   # folder holding the source code of all objects of a game,
                          # e.g. game_alien_shooter
...
|---|---|---|
| `scr_*` | Handler of a screen | `scr_game.cpp`, `scr_start.h` |
| `alien_*` | Object belonging to the Alien Shooter game | `alien_bullet.cpp`, `alien_enemy.h` |

Each game defines its own short prefix (for example `alien_*` for Alien Shooter) and applies it consistently to every file in that game's folder.
...
// app.h
#define AC_DISPLAY_GAME_UPDATE_INTERVAL (80)
#define AC_DISPLAY_START_BLINK_INTERVAL (500)
```

**Exception — system / project level macros:** macros belonging to the whole game (not tied to any specific object) use the project prefix `AC_DISPLAY_*`:
...
Group related constants in the right module header (`scr_game.h` holds game constants, `scr_start.h` holds start screen constants, and so on). Never leave magic numbers scattered across `.cpp` files.
...
|---|---|---|
| `<GAME>_<OBJECT>_<ACTION>` | Per-game signal | `AC_DISPLAY_GAME_UPDATE` |
...
/*****************************************************************************/
enum {
    AC_DISPLAY_GAME_UPDATE = AC_DISPLAY_SHOW_MODBUS_PULL_SLEEP + 1,
...
```cpp
{AC_TASK_DISPLAY_ID, TASK_PRI_LEVEL_4, task_display_handle},
```
...
void draw_enemy(int16_t x, int16_t y, uint8_t row, bool anim);
void boss_shoot();
bool hit_enemy(int16_t bx, int16_t by);
```
...
State belonging to a task's object should carry that object's name (`px`, `pb[i].on`, `e_alive`); do not stash cross-cutting state inside another module's `.cpp`.
...
```text
[ADD] alien start screen sequence
[ADD] mermaid game flow sequence diagram
[UPDATE] rename Start Screen signals to AC_DISPLAY_START_BLINK_TICK
[UPDATE] split Alien narrative into Setup/Per-tick/Reset
[UPDATE] enlarge runtime mermaid font size
[FIX] off-by-one in alien zigzag clamp
[REMOVE] unused p_alt_gun static in scr_game
...
[ADD] scr_start.cpp + scr_game.cpp + scr_game_over.cpp ... # too long, group by topic
```
